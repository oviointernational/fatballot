...
export const ProfilePage: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    if (myCandidateProfile) {
      (async () => {
        const { data: backers, error: backersErr } = await supabase.rpc('voters_for_candidate', {
          p_candidate_id: myCandidateProfile.id
        });
        if (backersErr) {
          setCandidateVotersError(backersErr.message || 'Permission denied');
          return;
        }
        const { data: voteRows } = await supabase
          .from('votes')
          .select('voter_ra_number,timestamp')
          .eq('candidate_id', myCandidateProfile.id);

        const timestampByRA: Record<string, string> = {};
        (voteRows || []).forEach((r: any) => {
          timestampByRA[String(r.voter_ra_number)] = r.timestamp;
        });

        const list = (backers || []).map((b: any) => {
          const fullName = b.full_name || '';
          const nameParts = fullName.split(/\s+/);
          return {
            voter: {
              id: '',
              raNumber: String(b.ra_number),
              email: '',
            ...
            timestamp: timestampByRA[String(b.ra_number)] || ''
          };
        });
        setCandidateVoters(list);
      })();
    }
  }, [user, myCandidateProfile]);
