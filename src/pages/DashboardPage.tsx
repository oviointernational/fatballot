...
export const DashboardPage: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    fetch('/api/voters').then(res => res.json()).then(setVotersList).catch(console.error);
    fetch('/api/ycec').then(res => res.json()).then(setYcecList).catch(console.error);
    fetch('/api/timeline').then(res => res.json()).then(setTimelineList).catch(console.error);
  }, []);
