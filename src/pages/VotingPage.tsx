...
export const VotingPage: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  const [selectedCandidateForModal, setSelectedCandidateForModal] = useState<CandidateProfile | null>(null);
  const [submittingVote, setSubmittingVote] = useState<boolean>(false);
  const [saveFeedback, setSaveFeedback] = useState<{ message: string; isChange?: boolean } | null>(null);
  ...
  const activeCandidates = candidates.filter(c => c.officeId === activeOffice?.id);
  const isSingleCandidate = activeCandidates.length === 1;

  // Current user's vote for the active office
  const currentVote = myVotes.find(v => v.officeId === activeOffice?.id);

  const handleVoteSubmit = async (choice: 'candidate' | 'for' | 'against', candidateId?: string) => {
    if (!user) {
      onNavigate('login');
      return;
    }
    ...
    if (res.success) {
      ...
      setSaveFeedback({
        message: res.isChange ? 'Ballot updated & auto-saved securely!' : 'Vote cast & auto-saved securely!',
    ...
    <div className="flex flex-col h-full overflow-hidden bg-gray-50 dark:bg-[#080C15]">
      ...
      {activeTab === 'ballot' ? (
        ...
        <div className="flex flex-1 h-full overflow-hidden">
          ...
          >
            ...
            <div className="w-full space-y-3">
              {offices.map((office) => {
                ...
                const hasVotedForThis = myVotes.some(v => v.officeId === office.id);
                ...
                return (
                  ...
                  >
                    {/* Circular Icon with vote indicator badge */}
          ...
          {/* 90% Right Side Container: Independent scroll up/down without interrupting other elements */}
          <main className="flex-1 h-full overflow-y-auto p-4 md:p-8 space-y-6">
            ...
            <div className="bg-white dark:bg-[#0F172A] border border-gray-200 dark:border-[#1E2E4E] rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-col items-start sm:items-end text-xs text-gray-500 dark:text-slate-400 space-y-1">
                <div className="flex items-center space-x-1.5 font-medium">
                  ...
                  <span>Ballots automatically recorded to immutable ledger</span>
                </div>
            ...
            <div className="space-y-4">
              {activeCandidates.length === 0 ? (
                ...
                /* Single Contestant Race: Vote FOR or AGAINST */
                (() => {
                  const candidate = activeCandidates[0];
                  const isVotedFor = currentVote?.choice === 'for';
                  const isVotedAgainst = currentVote?.choice === 'against';
                  const hasVotedThisOffice = Boolean(currentVote);

                  // Theme styling rules:
                  // If voted FOR: light material green (#E8F5E9 / dark #0F3020)
                  // If voted AGAINST: light material red (#FFEBEE / dark #321419)
                  // If unvoted: default theme
                  ...
                  return (
                    <div
                      key={candidate.id}
                      className={`rounded-3xl p-6 md:p-8 transition-all duration-300 shadow-sm border ${cardStyle}`}
                    >
                      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                        {/* Contestant Image, Name, Position, View Icon */}
                        <div className="flex items-start md:items-center space-x-5">
                          <img
                            src={candidate.avatar}
                            alt={candidate.name}
                            className="w-24 h-24 rounded-full object-cover ring-4 ring-white/50 dark:ring-slate-700/50 shadow-md shrink-0"
                          />
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="flex items-center space-x-2 text-xs font-mono text-gray-400">
                              <span>RA-{candidate.raNumber}</span>
                              <span>•</span>
                        ...
                        {/* Vote Actions (Vote FOR or Vote AGAINST) */}
                ...
                /* Multi-Contestant Race */
                /* Requirement: "All are by default having the look of the theme but If you vote for one, the user profile card becomes light material green and others becomes light material red. Votes are automatically saved." */
                activeCandidates.map((candidate) => {
                  const isVotedThisCandidate = currentVote?.candidateId === candidate.id && currentVote?.choice === 'candidate';
                  const hasVotedSomeoneElse = Boolean(currentVote && currentVote.choice === 'candidate' && currentVote.candidateId !== candidate.id);

                  let cardStyle = 'bg-white dark:bg-[#0F172A] border-gray-200 dark:border-[#1E2E4E]';
                  if (isVotedThisCandidate) {
                    cardStyle = 'voted-material-green border-2';
                  } else if (hasVotedSomeoneElse) {
                    cardStyle = 'unvoted-material-red border-2';
                  }

                  return (
                    <div
                      key={candidate.id}
                      className={`rounded-3xl p-5 md:p-6 transition-all duration-300 shadow-sm border ${cardStyle}`}
                    >
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        {/* Contestant details: Image, Name, Position, Small View Icon */}
                        <div className="flex items-center space-x-4">
                          <img
                            src={candidate.avatar}
                            alt={candidate.name}
                            className="w-16 h-16 rounded-full object-cover ring-2 ring-white/60 dark:ring-slate-700 shadow-sm shrink-0"
                          />
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center space-x-2">
                              ...
                              <button
                                ...
                                title="View candidate profile summary tabs"
                              >
                                <Eye className="w-4 h-4" />
                              </button>
                            </div>
                        ...
                        <div className="flex items-center space-x-3 w-full sm:w-auto justify-end">
                          ...
                          >
                            <VoteIcon className="w-4 h-4" />
                            <span>
                              {isVotedThisCandidate ? 'Selected Choice ✓' : 'Vote for Candidate'}
                            </span>
                          </button>
                        </div>
                      </div>
        ...
        <main className="flex-1 h-full overflow-y-auto p-4 md:p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {liveResults.map((off) => (
              <div
                key={off.officeId}
                className="bg-white dark:bg-[#0F172A] border border-gray-200 dark:border-[#1E2E4E] rounded-3xl p-5 shadow-sm space-y-4"
              >
                {/* Card Header: Position Title + Number to indicate total votes */}
                ...
                {/* Card Body: Name of candidate, %count, and #count */}
                <div className="space-y-4">
                        ...
                        <div key={cand.candidateId} className="space-y-3">
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between text-xs font-mono">
                              ...
                              <div className="space-x-2 text-gray-600 dark:text-slate-300">
                                <span className="font-bold text-emerald-600">{forPct}% (%count)</span>
                                <span>#{cand.forCount} (#count)</span>
                              </div>
                            </div>
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between text-xs font-mono">
                              ...
                              <div className="space-x-2 text-gray-600 dark:text-slate-300">
                                <span className="font-bold text-red-600">{againstPct}% (%count)</span>
                                <span>#{cand.againstCount} (#count)</span>
                              </div>
                            </div>
                            <div className="w-full h-2.5 bg-gray-100 dark:bg-slate-800 rounded-full overflow-hidden">
                              <div
      ...
      {selectedCandidateForModal && (
        <CandidateModal
          ...
          office={offices.find(o => o.id === selectedCandidateForModal.officeId)}
          onClose={() => setSelectedCandidateForModal(null)}
          onVote={() => handleVoteSubmit('candidate', selectedCandidateForModal.id)}
          isVoted={currentVote?.candidateId === selectedCandidateForModal.id}
        />
      )}
    </div>
  );
