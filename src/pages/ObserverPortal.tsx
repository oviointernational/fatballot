...
import { mapTimelineRow } from "../lib/mappers";
...
export const ObserverPortal: React.FC<ObserverPortalProps> = ({ token }) => {
  ...
  useEffect(() => {
    const verify = async () => {
      try {
        const { data, error } = await supabase.rpc('verify_observer', { p_token: token });
        if (error || !data) {
          setError("Invalid or expired observer link.");
          setVerified(false);
        } else {
          setObserverInfo(data);
          setVerified(true);
        }
      } catch {
...
const fetchData = async () => {
    try {
      const [results, s, timelineRes] = await Promise.all([
        fetchLiveResults(),
        fetchSystemStats(),
      ...
      if (s) setStats(s);
      if (timelineRes.data) setTimeline(timelineRes.data.map(mapTimelineRow));
