...
export const LogPage: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      const [logsRes, verifyRes] = await Promise.all([
        fetch("/api/audit-log", { headers: sessionToken ? { "x-session-token": sessionToken } : {} }),
        fetch("/api/audit-log/verify")
      ]);
  ...
  const fetchAgentLogs = async () => {
    if (!user?.agentCandidateId || !sessionToken) return;
    setAgentLoading(true);
    try {
      const res = await fetch(`/api/audit-log/agent-monitor/${user.agentCandidateId}`, {
  ...
  }, [isAgent, activeTab, sessionToken]);
